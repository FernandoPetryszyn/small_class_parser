...
trait RubyClassParser extends RegexParsers {
  ...
  lazy protected val method = ("def" ~> identifier <~ "(") ~ ((repsep(parameter.?,",") <~ ")" ) <~ ("{")) ~ (sentences.* <~ "}")  ^^ {case id ~ parameters ~ sentences => Method(id,parameters map (parameter => parameter.getOrElse(Empty)),sentences.toList)} 
